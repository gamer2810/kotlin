...
private fun isStdlibAddedByUser(
    ...
    // Sequence.flatMap is only available since Kotlin 1.4 (Gradle 7.0+)
    return sequenceOf(*sourceSets)
        .map { sourceSet ->
        ...
        .flatten()
        .map { it.allNonProjectDependencies() }
        .flatten()
        .any { dependency ->
...
private fun chooseStdlibJvmDependency(
    coreLibrariesVersion: Provider<String>
) = if (SemVer.satisfies(coreLibrariesVersion.get(), "<1.7.255")) {
    "kotlin-stdlib-jdk8"
} else {
    "kotlin-stdlib"
}
                    ...
                    .forEach { configuration ->
                        configuration.withDependencies { dependencies ->
                            ...
                            if (testRootDependency != null) {
                                ...
                                val testCapability = compilation.kotlinTestCapabilityForJvmSourceSet(tasks, objects)
